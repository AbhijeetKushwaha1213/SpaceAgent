"""
Phase 18 — Live Gemini Baseline Benchmark Runner.

Executes S1, S2, S3, S5, S6, S200 through the full Sentinel pipeline using
live Gemini 2.5 Flash via GeminiProvider.
Captures all inputs, raw LLM outputs, latencies, guardrail violations,
physics reconciliations, safety verdicts, and generates benchmark metrics.
"""
...
from app.agent.rag import initialize_pdf_rag, retrieve_procedures_traced, classify_chunk_text
from app.llm.provider import GeminiProvider, ProviderConfig
...
def main():
    print("=" * 70)
    print("PHASE 18 — REAL GEMINI 2.5 FLASH BASELINE EXPERIMENT")
    print("=" * 70)
    ...
    for sid in target_sids:
        ...
        if parsed_output is not None:
            ...
            final_output = LLMRankingOutput(
                ranked_hypotheses=[],
                reasoning_summary=f"Gemini call failed or produced invalid output: {provider_error}",
        ...
        top1_correct = (raw_top_fault == expected_fault) or (expected_fault == "INSUFFICIENT_EVIDENCE" and (raw_top_fault in ("INSUFFICIENT_EVIDENCE", "UNKNOWN", "NONE") or final_validated.requires_human_review))
        ...
        print(f"  Gemini Raw Top Hypothesis: {raw_top_fault} (Conf: {raw_top_conf:.2f})")
        print(f"  Deterministic Top Hypothesis: {hyp.top.fault_id if hyp.top else 'None'} (Score: {hyp.top.score if hyp.top else 0:.2f})")
        print(f"  Ground Truth: {expected_fault} | Correct: {top1_correct}")
        print(f"  Evidence Cited: {len(all_cited)} valid of {len(all_cited)} total (Valid: {ev_grounded})")
        print(f"  Procedures Selected: {selected_procs} (Valid: {procs_valid})")
        print(f"  Physics Consistency: {physics_consistent}")
        print(f"  Guardrail Violations: {[v.violation_type.value for v in guardrail_result.violations] if guardrail_result else []}")
        print(f"  Safety Status: {safety_val.safety_status.value} (Validated: {len(safety_val.validated_steps)}, Blocked: {len(safety_val.blocked_steps)})")
        print(f"  Latencies: LLM = {llm_latency:.3f}s, E2E = {e2e_latency:.3f}s")
        ...
        results[sid] = {
            ...
            "gemini_raw_top": raw_top_fault,
            "gemini_confidence": raw_top_conf,
            "gemini_reasoning": parsed_output.reasoning_summary if parsed_output else "",
            "gemini_selected_procs": list(selected_procs),
            "gemini_cited_evidence": list(all_cited),
    ...
    # Summary Statistics
    confidences = [r["gemini_confidence"] for r in results.values() if r["gemini_confidence"] > 0]
    correct_confs = [r["gemini_confidence"] for r in results.values() if r["top1_correct"] and r["gemini_confidence"] > 0]
    incorrect_confs = [r["gemini_confidence"] for r in results.values() if not r["top1_correct"] and r["gemini_confidence"] > 0]
    ...
    # Save output for report generation (NO SECRETS STORED)
    with open("gemini_baseline_raw.json", "w") as f:
        json.dump({"scenarios": results, "stats": summary_stats}, f, indent=2)
    print("Saved run data to gemini_baseline_raw.json")
